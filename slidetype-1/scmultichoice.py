...
def check_do(driver, slide_no, dest_dir):
    ...
    scorm_slide = {'type': 'multichoice', 'data': {'count': 0, 'value': -1, 'choice':[], 'header':''}}
    try:
        select_tag_element = slide_container.find_element(By.XPATH, ".//div[text()='Select one']")
    except NoSuchElementException as e:
        return False, ''
    
    h1_elements = slide_container.find_elements(By.XPATH, "//div[@data-contents]")
    ...
    time.sleep(1)
    check_button_element = slide_container.find_element(By.XPATH, ".//button[text()='Check']")
